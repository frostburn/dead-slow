...
const st=R.create(L[0]),events=[],command=R.command,update=R.update;
...
const recording={level:L[0].id,description:'Collect the waiting wagons, reverse clear of Quarry Junction and secure the complete train in the siding. Recorded power, brake, coupling and point commands; no repositioning.',duration:expectedTime+1,expectedTime,events};
fs.writeFileSync(path.join(__dirname,'../tests/fixtures/long-grade-1-controls.json'),JSON.stringify(recording,null,2)+'\n');
