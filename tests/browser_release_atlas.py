...
with sync_playwright() as p:
    ...
    for w in [7,8]:
        ...
        check(f'World {w} separates playable and planned missions',page.locator('.level-card:disabled').count()==(3 if w==7 else 12) and page.locator('.level-card:not(:disabled)').count()==(9 if w==7 else 0) and 'COMING SOON' in page.locator('#dialog').inner_text())
