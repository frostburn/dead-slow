...
test('the Century Ship is selectable but excluded from both relevant marathons',()=>{
    ...
    for(const id of ['meridian','grand-tour']) {t.marathon(id);assert.ok(t.state.marathon.route.every(i=>!L[i].bonus));assert.equal(t.state.marathon.route.length,id==='meridian'?12:72);}
