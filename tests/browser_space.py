...
def check_space(browser, check, html, screenshots=None):
    ...
    page.evaluate('DeadSlow.watch("yesterday",0);DeadSlow.step(220)')
    check('Chronogate replay displays its actual solid history',page.evaluate('DeadSlow.state().run.space.phase===1 && !!DeadSlow.state().run.space.echo && DeadSlow.state().run.space.loop.length>100'))
