...
with sync_playwright() as pw:
    ...
    check('Tour begins with eighty-four stages, no ranked departure and visible speed control',page.evaluate('DeadSlow.progress().circuit.length===84 && DeadSlowTest.state.storage.attempts===0') and page.locator('#review-controls').is_visible())
    ...
    for completed in [12,24,36,48,60,72,83]:
    ...
    check('Grand Tour ends with relief through and no extra bonus stage', 'The route is open.' in page.locator('#dialog').inner_text() and page.evaluate('DeadSlow.progress().circuit.completed===84 && DeadSlowTest.state.storage.races["grand-tour"].length===0'))
