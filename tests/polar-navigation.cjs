...
function navigate(n){
 ...
 const r=t.state.run;
 let phase=0,wp=0,docking=false,route=n===1?[...l.polar.route.slice(0,6),[660,235],[739,235]]:
 ...
 for(let tick=0;tick<120*(n===3?2500:850)&&t.state.status==='running';tick++){
  if(tick%30===0){
   ...
   if(n===1&&c.arrived&&phase<3){
    if(!phase)earlyPocket=I.pocketClear(r.polar);
    phase++;route=[[r.ship.x,r.ship.y],...(phase===1?[[745,320]]:phase===2?[[635,306],[578,306]]:[[635,292],[700,292],[746,292]])];wp=0;
   ...
   }else if(c.arrived&&(n===1&&phase===3||n===2||n===3&&phase===4))docking=true;
   if(docking){
