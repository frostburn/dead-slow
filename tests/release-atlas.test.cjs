...
const active=['coast','northwatch','archipelago','gerbozilla','long-grade','meridian'];
const altered=['gerbo-downhill','gerbo-forest-slalom','gerbo-prickly-business'];
const near=(a,b)=>assert.ok(Math.abs(a-b)<1e-7,`${a} != ${b}`);

test('eight-world catalog separates 76 playable assignments from 21 locked plans',()=>{
 ...
 assert.equal(L.length,76);assert.equal(L.catalog.length,97);assert.equal(new Set(L.catalog.map(l=>l.id)).size,97);
...
test('console rejects future missions/circuits without disturbing the current attempt',()=>{
 ...
 for(const w of [7,8]){
  assert.throws(()=>t.cheats.level(w,w===7?4:1),/Coming soon/);
 ...
 assert.equal(t.cheats.levels().filter(l=>l.comingSoon).length,21);
