...
(function(root){
    ...
    function prepare(level,command,action){
        ...
        $('polar-actions').hidden=!['survey','defense','strike'].includes(level.polar.mission);
        $('polar-tow-controls').hidden=level.polar.mission!=='survey';$('polar-gun-controls').hidden=!level.polar.gun;
    ...
    function render(canvas,level,run,zoom,options={}){
        ...
        if(op){
            for(const m of op.melts){
                const progress=m.total?m.opened/m.total:1;
                line(m.route,progress===1?'#78d9d080':'#9ddde0bb',2,[4,7]);
                const p=m.route[Math.floor(m.route.length/2)];label(m.name+' · '+Math.floor(progress*100)+'%',p[0],p[1]-20,'#b6e7de',9);
            }
            if(c.exclusion){
            ...
            if(op.survey){
                ...
                label(op.survey.recovered?'RECORDERS ABOARD':'RECOVER RECORDERS',op.survey.ship.x,op.survey.ship.y-32,'#f1d69b',9);
            }
        ...
        if(c.mission==='defense'){
            for(const f of st.fleet){const p=f.home.at(-1);line([[f.start[0],f.start[1]],p],'#8ce0b8',1.5,[5,5]);ctx.beginPath();ctx.ellipse(p[0],p[1],30,19,0,0,Math.PI*2);ctx.strokeStyle='#8ce0b8';ctx.lineWidth=2;ctx.stroke();}
        }else{
