...
(function(root) {
    ...
    function dialog(kind,level,run,format,records=[],settings={},archived=null,context={}) {
        ...
        if(kind==='log') return `${intro}<h1>Gerbozilla’s field log.</h1><table class="log-table"><thead><tr><th>TIME / IGT</th><th>DAMAGE EVENTS</th><th>CLASS</th></tr></thead><tbody>${records.length?records.map(r=>`<tr><td>${format(r.time)}</td><td>${r.contacts}</td><td>${r.clean?'CLEAN':'OPEN'}</td></tr>`).join(''):'<tr><td colspan="3">No completed course yet.</td></tr>'}</tbody></table><p class="subtle">Clean means zero shell damage. Records and ghosts are local. The twelve-course World 5 circuit and sixty-stage Grand Tour have separate overall and clean records.</p>${archives.filter(a=>a.runs?.length).map(a=>`<details><summary>${a.id?.endsWith('-preview')?'Preview field map':'Earlier terrain'} records (archived)</summary><p class="subtle">This layout differs from the active course. Its own ghost and splits remain in exports.</p>${a.runs.map(r=>`<p>${format(r.time)} · ${r.clean?'CLEAN':'OPEN'}</p>`).join('')}</details>`).join('')}
